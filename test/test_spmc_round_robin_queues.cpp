...
#include <string>
#include "q/q_api.hpp"
#include <q/queue_spmc_fixed_sender.hpp>
...
TEST(SingleProducer_MultipleConsumers, CreateOneQueue) {
   ...
   spmc::fixed_size::round_robin::Sender<qtype> producer({temporary});
...
TEST(SingleProducer_MultipleConsumers, CreateManyQueues) {
   using element = std::string;
   using qtype = spsc::circular_fifo<element>;
   ...
   spmc::fixed_size::round_robin::Sender<qtype> consumer({senders});
   EXPECT_EQ(kSizeTotal, consumer.capacity());
   EXPECT_EQ(kSizeTotal, consumer.capacity_free());
}

TEST(SingleProducer_MultipleConsumers, push) {
   using element = std::string;
   using qtype = spsc::circular_fifo<element>;
   ...
   // convert the setup to a MPSC setup
   //
   spmc::fixed_size::round_robin::Sender<qtype> producer({s1, s2});
   std::string arg = "s0";
   producer.push(arg);
